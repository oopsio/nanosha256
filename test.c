...
int main() {
    ...
    // Test 3: 56-byte NIST string 
    
    // NIST "Long string" (Standard FIPS 180-4 Test Case 2)
    // "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmonlopnopq" 

    const char *long_str = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmonlopnopq";

    printf("\nRunning Test 3 (56-byte string):\n");
    SHA256_Init(&ctx);
    SHA256_Update(&ctx, long_str, strlen(long_str));
    SHA256_Final(hash, &ctx);
